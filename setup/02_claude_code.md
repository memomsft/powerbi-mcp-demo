...
> ⚠️ Si editaste `~/.claude.json` usando PowerShell `ConvertTo-Json`, el archivo puede quedar con `\\\\` en lugar de `\\`. Ambos funcionan — no es necesario corregirlo. Si editas manualmente en VS Code, usa `\\` simple.
